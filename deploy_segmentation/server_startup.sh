...
git clone -b dp_and_cloud https://${token}@github.com/SanoScience/FederatedLearning_MSc.git
sudo chmod -R 777 FederatedLearning_MSc
cd FederatedLearning_MSc
pip3 install -r requirements.txt --user
cd segmentation

echo 'Running FL'
export PYTHONPATH=$(dirname $PWD)
echo "export PYTHONPATH=$(dirname $PWD) && python3 server_segmentation.py --c ${node_count} --r ${rounds} --a ${algo} --le ${le} --lr ${lr} --bs ${bs} --o ${opt} --ff ${ff} --mf ${mf} --nl 1.0" > run.sh
